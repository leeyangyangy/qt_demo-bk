...
TriggerMonitor::TriggerMonitor(QObject *parent, Widget *widget)
    : QObject(parent),
      ...
      m_widget(widget),
      m_isTaskRunning(false),  // 轮询定时器超时回调，检查是否满足触发同步的条件
...
void TriggerMonitor::showSettingsDialog() {
  ...
  const auto layout = new QFormLayout(&dialog);
  ...
  // 新增：上次同步时间设置（允许用户修改）
  QDateTimeEdit *lastSyncEdit = new QDateTimeEdit(
  ...
  // 新增：触发频率（单位：分钟）控制
  QSpinBox *frequencySpin = new QSpinBox(&dialog);
  ...
  // 新增：每周触发日设置
  QGroupBox *weekDaysGroup = new QGroupBox("每周触发日(可选)", &dialog);
  QHBoxLayout *weekLayout = new QHBoxLayout(weekDaysGroup);
  QCheckBox *monCheck = new QCheckBox("周一", weekDaysGroup);
  QCheckBox *tueCheck = new QCheckBox("周二", weekDaysGroup);
  QCheckBox *wedCheck = new QCheckBox("周三", weekDaysGroup);
  QCheckBox *thuCheck = new QCheckBox("周四", weekDaysGroup);
  QCheckBox *friCheck = new QCheckBox("周五", weekDaysGroup);
  QCheckBox *satCheck = new QCheckBox("周六", weekDaysGroup);
  QCheckBox *sunCheck = new QCheckBox("周日", weekDaysGroup);
  weekLayout->addWidget(monCheck);
  weekLayout->addWidget(tueCheck);
  weekLayout->addWidget(wedCheck);
  weekLayout->addWidget(thuCheck);
  weekLayout->addWidget(friCheck);
  weekLayout->addWidget(satCheck);
  weekLayout->addWidget(sunCheck);
  ...
  if (m_triggerWeekDays.isEmpty()) {
    ...
    monCheck->setChecked(m_triggerWeekDays.contains(Qt::Monday));
    tueCheck->setChecked(m_triggerWeekDays.contains(Qt::Tuesday));
    wedCheck->setChecked(m_triggerWeekDays.contains(Qt::Wednesday));
    thuCheck->setChecked(m_triggerWeekDays.contains(Qt::Thursday));
    friCheck->setChecked(m_triggerWeekDays.contains(Qt::Friday));
    satCheck->setChecked(m_triggerWeekDays.contains(Qt::Saturday));
    sunCheck->setChecked(m_triggerWeekDays.contains(Qt::Sunday));
  }

  // 同步时间设置（原先用于每天某个时间触发，可留作备用）
  auto *timeEdit = new QTimeEdit(syncTime, &dialog);
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    // 更新每周触发日
    m_triggerWeekDays.clear();
    if (monCheck->isChecked()) m_triggerWeekDays.insert(Qt::Monday);
    if (tueCheck->isChecked()) m_triggerWeekDays.insert(Qt::Tuesday);
    if (wedCheck->isChecked()) m_triggerWeekDays.insert(Qt::Wednesday);
    if (thuCheck->isChecked()) m_triggerWeekDays.insert(Qt::Thursday);
    if (friCheck->isChecked()) m_triggerWeekDays.insert(Qt::Friday);
    if (satCheck->isChecked()) m_triggerWeekDays.insert(Qt::Saturday);
    if (sunCheck->isChecked()) m_triggerWeekDays.insert(Qt::Sunday);
  ...
  for (const QJsonValue &value : rows) {
    ...
    connect(task, &SyncTask::taskCompleted, this,
            [this, taskCount, totalTasks](const QString &source,
              ...
              //         progress;
              if (m_widget) _sleep(100);  // 睡眠100ms
